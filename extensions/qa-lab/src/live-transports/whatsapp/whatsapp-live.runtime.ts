...
} from "@openclaw/whatsapp/qa-driver-api.js";
...
type WhatsAppStructuredInboundDriver = Pick<
  ...
  "sendContact" | "sendLocation" | "sendMedia" | "sendPoll" | "sendSticker"
>;
...
async function runWhatsAppStructuredInboundChecks(params: {
  ...
  await params.driver.sendPoll(params.target, {
    question: `Reply with only this exact marker after seeing this poll: ${params.pollToken}`,
    options: ["yes", "no"],
  });
  await params.waitForStructuredReply("poll", pollStartedAt, params.pollToken);
